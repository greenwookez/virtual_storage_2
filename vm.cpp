...
void OS::Requester::AddRequest(Process* p_process, VirtualAddress vaddress, bool load_flag) {
    if (IsEmpty()) {
        /* ≈сли на момент добавлени€ запроса в очередь, последн€€ пуста, то
...
void OS::Substitute(VirtualAddress vaddress, Process* p_process) {
    /* ¬ыбор кандидата дл€ распределени€. ¬ данном случае - рандомно. */
...
int randomizer(int max) { // ‘ункци€, возвращающа€ любое число от 0 до max-1.
